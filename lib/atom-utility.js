...
import { setEventListener, setEventListenerClickAndEnter, createElementWithClass, isNode } from './html-utility'
...
//
export function makeMiniEditorAndSetEditorDidChange(className, text, obj, propertyName, subscriptions) {
  miniEditor = makeMiniEditorWithView(text, className);
  subscriptions.add(
...
//  search : 消去ボタンが出る
//  textarea : 広さを変えられる
export function makeInputText(type, classes, placeholder, defaultValue, func, useCapture, listners) {
  let workClasses = [];
  if(classes !== null) {
...
// title: string
// checked : bool
export function makeCheckbox(classes, content, checked, func, useCapture, listners) {
...
//
export function makeInputNumber(classes, placeholder, min, max, defaultValue, func, useCapture, listners) {
...
//
export function makeSelector(classes, options, defaultValue, func, useCapture, listners) {
...
//icon : string
//toggle: bool
export function makeButton(classes, content, selected, size, color, inlineType, icon, inner, toggle, func, useCapture, listners) {
  let workClasses = ['btn'];
...
export function getEditorDecoratedMarker(markers, editor) { //sort?
  let target = null
  if(isEmpty(editor)) {
  ...
  for(marker of markers) {
    for(activeMarker of activeMarkers) {
...
export function destroyActiveDecoratedMarker(markers) {
...
export function logRange(range) {
  console.log('start_row: ', range.start.row, 'start_column: ', range.start.column,
    '--- end_row: ', range.end.row, 'end_column: ', range.end.column);
}

export function getSelectedMarkerBufferRange(range, marker, markers) {
  let markerRange = null;
  if(marker) {
    markerRange = marker.getBufferRange();
    console.log('##getSelectedMarkerBufferRange1');
    logRange(markerRange);
    logRange(range);
  ...
  for(work of markers) {
    ...
    console.log('##getSelectedMarkerBufferRange2');
    //console.log(work);
    //logRange(markerRange);
    //logRange(range);
    if(markerRange.isEqual(range)){
...
export function getChangedConfig(obj, property){
  return (evt) => {
    obj[property] = evt.newValue;
  };
}

export function appendNewLine(editor, num) {
