...
import { isEmpty, getReversedNewArray, countCaptureGroup } from './utility';
...
export default {
  config: {
    ...
    autoFocusPosition: {
      ...
      'default': 'prefix',
      enum: [
        'prefix', 'suffix', 'ignoreCase', 'targetType', 'customTarget', 'place', 'paddingChar', 'align',
        'outputBase', 'upperCase', 'start', 'end', 'increment', 'add', 'multiply', 'mark', 'first', 'middle', 'last',
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add(this.editNumbersView.rootElement, {
      ...
      'edit-numbers:mark': (evt) => this.preCall('mark'),
  ...
  mark(editor) {
    ...
    let activeMarkers = [];
    editor.scan(
      this.conditions.getRegex(false),
