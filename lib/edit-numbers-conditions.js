...
import { isEmptyString, paddingNumber, getMatchRange, enclose, countCapturingGroup } from './utility';
...
export default class ENConditions {
  constructor(conditions) {
    ...
    this.place = conditions && conditions.hasOwnProperty('place') ? conditions.place : 1; //number
  ...
  setRegex(string, property) {
    try {
      ...
      atom.notifications.addError('Invalid regular expression.' + property, {detail: err, dissmiss: false});
      this[property] = '';
  ...
  getNumberString(numberString) {
    ...
    if(!isEmptyString(numberString) && this.editType === 'modify') {
      let targetBase = this.targetType === 'hexadecimal' ? 16 : 10;
      num += parseInt(numberString, targetBase);
    }
    num = num * this.multiply + this.add + this.currentNumber;
    return paddingNumber(num, this.base, this.place, this.upperCase, this.paddingChar, this.align);
  }
  ...
  getRegex(doEnclose) { //単純に連結してしまったら当初の目的を果たせない?
    ...
    result = new RegExp(regex, this.getRegexFlag());
    return result;
  }
