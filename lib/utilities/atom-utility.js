...
//search : 消去ボタンが出る
//textarea : 広さを変えられる
function makeInputText(type, classes, digitsholder, defaultValue, func, useCapture, listners) {
  let workClasses = [];
  if (classes !== null) {
  ...
  ele.type = type;
  if (digitsholder !== null) {
    ele.digitsholder = digitsholder;
  }
...
//
function makeInputNumber(classes, digitsholder, min, max, defaultValue, func, useCapture, listners) {
